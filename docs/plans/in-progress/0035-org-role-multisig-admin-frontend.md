...
| Path | Action | Notes |
|------|--------|-------|
| `frontend/src/components/OrgRegistrationForm.jsx` | add | the application form, grouped into the seven sections |
...
- [x] **5.** Build `OrgRegistrationForm.jsx` with the seven sections agreed in the design conversation: **(1)** organization identity — legal name, registration number, tax/VAT ID, address, founded date; **(2)** trade qualification — craftsman certificate, chamber membership number, specialization; **(3)** insurance — provider, policy number, coverage expiry; **(4)** contact person — name, role, business email, phone; **(5)** wallet — address plus the signature from task 6; **(6)** supporting evidence — website, references (free text only); **(7)** declarations — accuracy checkbox, fraud-ban acknowledgement, typed name and date. Document uploads are **not** form fields: each section that expects a document shows an instruction to attach it to the email. MUI v5 components throughout, matching the existing panels.
...
- [x] **12.** Manual verification of the application flow: fill the form, sign the challenge, confirm the signature recovers to the connected address, open the mail client, and confirm the body arrives intact and complete. *(Verified via the same headless-Chromium harness: filled all 7 sections, `personal_sign` succeeded against a real Hardhat dev-account signature, the signed confirmation and "Open email to submit" / "Copy application text" both appeared, and editing a field after signing correctly cleared the signature. The generated `mailto:` link was inspected directly and contains every field, the challenge, and the signature intact.)*
