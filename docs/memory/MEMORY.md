...
- [web3.js v4 returns event blockNumber/logIndex as BigInt](frontend/web3js-v4-bigint-event-fields.md) — sorting/subtracting them directly throws; wrap in `Number(...)` first (found in `getOrgRoleHolders`, ADR 0035).
...
- [CRT is meant to be pay-to-read](contracts/crt-pay-to-read-intent.md) — readers buy tokens to look up a VIN; fees refill the reward pool. Blocked: reads can't be gated without encrypting metadata (ADR 0036, D1).
- [Product roadmap: ADRs 0029-0033 (draft)](contracts/product-roadmap-0029-0033.md) — 5 draft trios from a 2026-07-31 wishlist: ownership history/public lookup, structured vehicle record, simple transfer, escrow/marketplace, notifications. 0029/0031/0032/0033 still draft, unimplemented; 0030's plan trio moved to `rejected/` 2026-08-04 (absorbed into ADR 0035).
