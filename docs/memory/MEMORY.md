...
- [VinCidRegistry is now a UUPS proxy](contracts/vincidregistry-uups-proxy.md) — bootstrap via `deploy.js`, all later changes via `scripts/upgrade.js`; registry address stable, data survives upgrades (ADR 0028). Sepolia rollout still pending.
