...
5. Sign for real. Expect a green confirmation and a "Submit" section to
   appear with "Open email to submit" and "Copy application text".
6. Edit any field after signing — expect the signature to clear (re-signing
   required before the submit section reappears).
7. Click "Open email to submit". Expect the OS/browser mail client opens
   ...
   challenge and signature. **Check whether the body is truncated** — if
   so, this is the known `mailto:` length limit (decision
   `2026-08-03-001`); use "Copy application text" instead and note the
   truncation point as a plan amendment.
8. Click "Copy application text" and paste into a text editor — confirm the
   full body is present and matches what would be emailed.
9. Switch MetaMask accounts between signing and this step — expect the
   stale signature to already be cleared per step 6, so nothing mismatched
   can be submitted silently.
